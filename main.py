...
def main(args):
    if args.use_wandb:
        wandb.init(
            ...
            name=args.run_name,
            config=vars(args),
            settings=wandb.Settings(init_timeout=300)  # 默认是90秒，这里调到300秒
        )
    ...
    if args.label_feats and args.num_label_hops >= 3:
        ...
        if os.path.exists(p):
            ...
            print(f"Warning: {p} does not exist, self_value_dict will be None.")
        print("self_value_dict",self_value_dict)
    else:
        self_value_dict = None
    # =======
    ...
    if os.path.exists(f'{args.dataset}_feats.pt') and args.lm:
        print(f'Load feats from {args.dataset}_feats.pt')
        feats = torch.load(f'{args.dataset}_feats.pt', map_location='cpu')
        tgt_type = "P"
    ...
    if( not os.path.exists(checkpt_folder)):
        ...
        # 保存参数到 JSON 文件
    import json
    args_path = os.path.join(checkpt_folder, 'args.json')
    ...
    for stage in range(args.start_stage, len(args.stages)):
        ...
        for epoch in range(epochs):
            ...
            if epoch % args.eval_every == 0:
                ...
                if val_acc > best_val_acc:
                    ...
                    save_pandas('./predictions_emb_1.csv',preds[valid_node_nums:valid_node_nums+test_node_nums])
                    count = 0
                else:
        ...
        os.rename('./predictions_emb_1.csv', f'./predictions_emb_1_{best_val_acc}.csv')
...
def parse_args(args=None):
    ...
    parser.add_argument('--wandb-project', type=str, default='hgamlp',
