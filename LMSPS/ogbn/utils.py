...
def load_mag2(args, symmetric=True):
...
def load_mag(args,symmetric=True):
    # Load the masked data from the .pt file
    dataset = torch.load("/home/wanglonghao/wanglonghao_space/Projects/RGCN/data_masked.pt", weights_only=False)  # Assuming args.data_path is the path to 'data_masked.pt'
    ...
    # Add node features for 'paper' nodes
    features = torch.tensor(x_dict['paper'], dtype=torch.float32).squeeze()  # Squeeze to remove the extra dimension
    g.nodes['paper'].data['feat'] = features

    # Add labels for 'paper' nodes
    labels = torch.tensor(y_dict['paper'], dtype=torch.long).squeeze()  # Squeeze to remove the extra dimension
    g.nodes['paper'].data['label'] = labels

    # Add features for 'author' nodes (initialize with random features if not available)
    if 'author' in x_dict:
        author_emb = torch.tensor(x_dict['author'], dtype=torch.float32).squeeze()
    else:
        author_emb = torch.Tensor(g.num_nodes('author'), args.embed_size).uniform_(-0.5, 0.5)
    g.nodes['author'].data['feat'] = author_emb

    # Add features for 'institution' nodes (initialize with random features if not available)
    if 'institution' in x_dict:
        institution_emb = torch.tensor(x_dict['institution'], dtype=torch.float32).squeeze()
    else:
        institution_emb = torch.Tensor(g.num_nodes('institution'), args.embed_size).uniform_(-0.5, 0.5)
    g.nodes['institution'].data['feat'] = institution_emb

    # Add features for 'field_of_study' nodes (initialize with random features if not available)
    if 'field_of_study' in x_dict:
        topic_emb = torch.tensor(x_dict['field_of_study'], dtype=torch.float32).squeeze()
    else:
        topic_emb = torch.Tensor(g.num_nodes('field_of_study'), args.embed_size).uniform_(-0.5, 0.5)
    g.nodes['field_of_study'].data['feat'] = topic_emb

    # Prepare the split indices for train, validation, and test sets
    train_nid = torch.tensor(split_idx['train']['paper'], dtype=torch.long).squeeze()  # Ensure correct shape (629571,)
    val_nid = torch.tensor(split_idx['valid']['paper'], dtype=torch.long).squeeze()  # Ensure correct shape (64879,)
    test_nid = torch.tensor(split_idx['test']['paper'], dtype=torch.long).squeeze()  # Ensure correct shape (41939,)
    ...
    # Now apply the degree filtering and symmetric adjacency if needed
    adjs = []

    # Degree-based filtering (if required)
    out_max_degree = args.out_max_deg  # Max out-degree
    in_max_degree = args.in_max_deg   # Max in-degree

    # Apply degree filtering for each edge type
    for i, etype in enumerate(g.etypes):
        src, dst, eid = g._graph.edges(i)
        edge_keep = None
    ...
    # Assign features to new graph
    new_g.nodes['P'].data['P'] = g.nodes['paper'].data['feat']
    new_g.nodes['A'].data['A'] = g.nodes['author'].data['feat']
    new_g.nodes['I'].data['I'] = g.nodes['institution'].data['feat']
    new_g.nodes['F'].data['F'] = g.nodes['field_of_study'].data['feat']

    # Compute the adjacency matrices for various edge types
    IA, PA, PP, FP = adjs

    # Save the computed diagonal terms
    diag_name = f'{args.dataset}_PFP_diag.pt'
    if not os.path.exists(diag_name):
        PF = FP.t()
        PFP_diag = sparse_tools.spspmm_diag_sym_ABA(PF)
    ...
    return new_g, labels, new_g.num_nodes('P'), n_classes, train_nid, val_nid, test_nid, evaluator
