...
def load_mag2(device):
    ...
    print(target_node_type, feature_node_types, labels.shape, n_classes, train_nid.shape, val_nid.shape, test_nid.shape)

    test_labels = labels[test_nid]
    print("Test set labels:", test_labels.min(), test_labels.max())
    print("test set size", test_nid.shape)
    exit()
    return g, target_node_type, feature_node_types, labels, n_classes, train_nid, val_nid, test_nid

def load_mag(device):
    # Load the data from the .pt file
    dataset = torch.load('/home/wanglonghao/wanglonghao_space/Projects/RGCN/data_masked.pt', weights_only=False)

    # Extract components from the dataset
    edge_index_dict = dataset['edge_index_dict']
    num_nodes_dict = dataset['num_nodes_dict']
    x_dict = dataset['x_dict']  # Features
    y_dict = dataset['y_dict']  # Labels
    split_idx = dataset['split']  # Train, valid, test splits

    # Convert edge_index_dict into the format required by DGL (list of tuples for each edge type)
    edge_index_dict_dgl = {}
    for k, v in edge_index_dict.items():
        if isinstance(v, np.ndarray):
            # Convert np.ndarray to a list of tuples (source, target)
            edge_index_dict_dgl[k] = [(int(src), int(dst)) for src, dst in zip(v[0], v[1])]
        elif isinstance(v, torch.Tensor):
            # If it's already a tensor, convert to list of tuples (source, target)
            edge_index_dict_dgl[k] = [(int(src), int(dst)) for src, dst in zip(v[0].tolist(), v[1].tolist())]

    # Create the graph (heterogeneous graph)
    g = dgl.heterograph(edge_index_dict_dgl, num_nodes_dict=num_nodes_dict)

    # Add node features (for 'paper' nodes in this case)
    features = torch.tensor(x_dict['paper'], dtype=torch.float32).squeeze().to(device)  # Squeeze to remove the extra dimension
    g.nodes['paper'].data['feat'] = features

    # Add labels for 'paper' nodes
    labels = torch.tensor(y_dict['paper'], dtype=torch.long).squeeze().to(device)  # Squeeze to remove the extra dimension
    g.nodes['paper'].data['label'] = labels

    # Prepare the split indices for train, validation, and test sets
    train_nid = torch.tensor(split_idx['train']['paper'], dtype=torch.long).squeeze().to(device)  # Ensure correct shape (629571,)
    val_nid = torch.tensor(split_idx['valid']['paper'], dtype=torch.long).squeeze().to(device)    # Ensure correct shape (64879,)
    test_nid = torch.tensor(split_idx['test']['paper'], dtype=torch.long).squeeze().to(device)    # Ensure correct shape (41939,)

    # Get number of classes
    n_classes = dataset['num_classes']

    # Define target node type and feature node types
    target_node_type = "paper"
    feature_node_types = [target_node_type]
...
def load_data_masked(device):
    # Load the data from the .pt file
    data = torch.load('/home/wanglonghao/wanglonghao_space/Projects/RGCN/data_masked.pt')

    # Extract components from the dataset
    num_nodes_dict = data['num_nodes_dict']
    edge_index_dict = data['edge_index_dict']
    x_dict = data['x_dict']  # Features
    y_dict = data['y_dict']  # Labels
    split_idx = data['split']  # Train, valid, test splits

    # Get the 'paper' node type as the target node type
    target_node_type = 'paper'
    feature_node_types = [target_node_type]
    ...
    # Add node features (for 'paper' nodes in this case)
    features = torch.tensor(x_dict[target_node_type], dtype=torch.float32).to(device)
    g.nodes[target_node_type].data['feat'] = features

    # Add labels for 'paper' nodes
    labels = torch.tensor(y_dict[target_node_type], dtype=torch.long).to(device)
    g.nodes[target_node_type].data['label'] = labels

    # Prepare the split indices for train, validation, and test sets
    train_nid = torch.tensor(split_idx['train']['paper'], dtype=torch.long).to(device)
    val_nid = torch.tensor(split_idx['valid']['paper'], dtype=torch.long).to(device)
    test_nid = torch.tensor(split_idx['test']['paper'], dtype=torch.long).to(device)

    # Number of classes
    n_classes = data['num_classes']

    # Return graph and splits in the same format as other datasets
    return g, target_node_type, feature_node_types, labels, n_classes, train_nid, val_nid, test_nid
...
def load_dgl_data(dataset, use_all_feat=False, embedding_size=None, use_nrl=False, random_state=None):
    ...
    elif dataset == "data_masked":
        # Use the new load function for data_masked
        hetero_graph, target_node_type, feature_node_types, labels, n_classes, train_index, valid_index, test_index = load_data_masked(device="cpu")
        num_epochs = 200  # Adjust according to your needs
        patience = 30  # Adjust according to your needs
        batch_size = 3000  # Adjust batch size if necessary
        validation_freq = 10  # Adjust validation frequency if necessary
        convert_to_tensor = False  # Adjust if needed, depending on whether you want tensors
