...
class TorchTrainModel(nn.Module):
    ...
    def fit(self, train_data, 
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            if (epoch + 1) % validation_freq == 0:
                ...
                if(validation_logs["val_accuracy"] > best_val_acc):
                    best_val_acc = validation_logs["val_accuracy"]
                    print("best_val_acc = {}".format(best_val_acc))
                    test_y_pred = self.predict(test_data).argmax(dim=-1, keepdim=True)
                    self.save_pandas("./test_y_pred.csv", test_y_pred)
    ...
    def save_pandas(self,root,y_pred):
